...
import com.web.forumTunisia.comment.CommentRepository;
...
public class StatisticsService {
 ...
 private final CommentRepository commentRepository;
    ...
    private Map<String, Map<Category, Long>> getDailyStatistics(Date startDate, Date endDate, String category) {
        ...
        // Afficher la valeur initiale de startDate
        System.out.println("Initial startDate: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(startDate));

        // Configure le calendrier pour la startDate
        ...
        // Configure le calendrier pour la endDate
        ...
        if (periodEnd.after(now.getTime())) {
            periodEnd = now.getTime();
            System.out.println("Adjusted periodEnd to current time: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(periodEnd));
        }

        // Traitement des jours de la période
        while (periodStart.before(periodEnd) || periodStart.equals(periodEnd)) {
            ...
            dailyStats.put(key, getStatisticsByPeriod(periodStart, periodStart, category, PeriodType.HOUR));
...
/////////////////////////////////////////comment //////////////////////////////////////////////////////
    ...
    private Map<String, Map<Category, Long>> getDailyCommentStatistics(Date startDate, Date endDate, String category) {
        ...
        while (periodStart.before(periodEnd) || periodStart.equals(periodEnd)) {
            ...
            String key = new SimpleDateFormat("EEE dd MMM yyyy", Locale.FRENCH).format(periodStart);
            System.out.println("Processing day: " + key);
            dailyStats.put(key, getStatisticsCommentByPeriod(periodStart, periodStart, category, PeriodType.HOUR));
...
/////////////////////////////////////////groupe //////////////////////////////////////////////////////
    ...
    private Map<String, Map<Category, Long>> getDailyGroupeStatistics(Date startDate, Date endDate, String category) {
        ...
        while (periodStart.before(periodEnd) || periodStart.equals(periodEnd)) {
            ...
            String key = new SimpleDateFormat("EEE dd MMM yyyy", Locale.FRENCH).format(periodStart);
            System.out.println("Processing day: " + key);
            dailyStats.put(key, getStatisticsGroupeByPeriod(periodStart, periodStart, category, PeriodType.HOUR));
    ...
    public List<Map<String, Object>> getUserStatisticsByGranularity(Date startDate, Date endDate, String granularity) {
        ...
        switch (granularity.toLowerCase()) {
            ...
            case "week":
                while (calendar.getTime().before(endDate)) {
                    Map<String, Object> dataPoint = new HashMap<>();
                    String weekYear = String.format("%d-W%02d", calendar.get(Calendar.YEAR), calendar.get(Calendar.WEEK_OF_YEAR));
                    long count = users.stream()
                            .filter(user -> isSameWeek(user.getDateAuthenticated(), calendar.get(Calendar.YEAR), calendar.get(Calendar.WEEK_OF_YEAR)))
                            .count();
                    double percentage = (users.size() > 0) ? (count * 100.0 / users.size()) : 0.0;
                    dataPoint.put("period", weekYear);
                    dataPoint.put("percentage", percentage);
                    dataPoint.put("total", count);  // Ajout du nombre total
